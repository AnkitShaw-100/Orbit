...
export default function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-30 flex items-center justify-between border-b border-line bg-void/85 px-6 py-4 backdrop-blur-md sm:px-10 2xl:px-16">
      <div className="flex items-center gap-10">
        <Link to="/" className="flex items-center gap-2 rounded-sm focus-visible:ring-2 focus-visible:ring-brand focus-visible:outline-none">
      ...
      <button
        ...
        className="rounded-sm text-foreground md:hidden focus-visible:ring-2 focus-visible:ring-brand focus-visible:outline-none"
      >
        {open ? <X className="size-6" /> : <Menu className="size-6" />}
      </button>

      {open && (
        <div className="absolute inset-x-4 top-full z-20 rounded-2xl border border-line bg-panel p-5 md:hidden">
