...
export function Failed({ error, onRetry }) {
  ...
  return (
    <div className="px-5 py-14 text-center">
      ...
      <p className="mx-auto mt-1.5 max-w-[46ch] text-xs leading-relaxed text-white/45">
        {offline
      ...
      {onRetry && (
        <button
          type="button"
          onClick={onRetry}
          className="mt-5 rounded-full border border-line px-5 py-2.5 text-xs font-medium text-white/75 transition-colors hover:border-white hover:text-white"
        >
          Try again
        </button>
      )}
